# EditingApp — AI Photo & Video Tools
...
**AI Photo to Video** adds four movements, a durable **Kling v3 Standard** queue through fal, private MP4 downloads, original comparison, resumable history and a portrait-to-video shortcut. Start with [the single new SQL file and activation guide](docs/VIDEO_SETUP.md) and [competitor research and cost calculations](docs/PHOTO_VIDEO_RESEARCH_20260913.md). There are no new required environment-variable names. Videos need the credit system and the video migration; incomplete setup disables submission honestly.
...
- Eleven public pages with canonical metadata, sitemap, robots, Open Graph previews, WebSite/WebApplication and guide Article/breadcrumb structured data. The private account page is excluded from indexing.
...
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FAL_KEY`                   | Server fal key with credits and access to Sunburst edit and Kling v3 standard image-to-video/motion-control. Video payload deletion needs admin scope on the same key. A separate OpenAI key is not used. |
...
1. Create a Supabase project in an appropriate region. Fresh installations apply **all four migrations in filename order** from `supabase/migrations/` using SQL Editor, or link the correct CLI project and run `supabase db push`. Existing installations apply only missing files. If billing/credits already works, the only new file is `202609130002_photo_video.sql`. See [video setup](docs/VIDEO_SETUP.md).
...
5. Confirm the `portraits` and `videos` buckets are **private**. Migrations apply restrictive policies denying browser access even if other bucket policies exist. All media access starts at authenticated server routes. Video delivery redirects to short-lived storage links for playback/range support. Job mutations and reservation RPCs are service-role only.
...
Stripe checkout and subscriptions activate after billing setup. There is no public gallery or automatic social posting. Welcome image credits are operator funded; video generation requires sufficient credits and a configured video studio.

## Retry and failure behavior

The browser persists a UUID before submission. A database RPC atomically grants the first caller permission to dispatch. Repeated IDs return the existing job; conflicting payloads are rejected. Active identical payloads with different IDs also reuse the existing job. Each dispatch performs exactly one billable fal POST with automatic retries disabled. On timeout or ambiguous provider failure the job is `uncertain`. A result written to storage before a database failure can be recovered. Direct image requests disable payload history and cannot recover a provider result that was never saved locally. Video requests use the durable queue: status/result GETs and signed callbacks recover an existing request, never generate again. See [video setup](docs/VIDEO_SETUP.md) for private delivery, retention, cleanup and operator reconciliation.
...
These commands are read-only and use `FAL_KEY` from `.env.local`. They return candidates and documentation links; they do not generate media or activate arbitrary models. Follow [Adding fal features](docs/ADDING_FAL_FEATURES.md) to register a future tool with its own schema and usage rules. The new video workflow provides the durable queue, signed callbacks and leased worker example. There is no automatic provider fallback.
