...
Keep existing `FAL_KEY`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, and the exact `APP_URL`. Add `STRIPE_SECRET_KEY` using a **test** key to the ignored `.env.local`, then run:

```sh
npm run billing:setup
```

The script creates or verifies three products, six recurring USD prices, and a portal configuration. For HTTPS APP_URL it also creates the webhook if absent. It never creates a customer subscription or charges a card. Re-running checks existing configuration instead of duplicating prices. Settings are written to **`.env.stripe.generated`**, which is Git-ignored. Copy its values into local/server environment settings; do not commit or upload this file publicly.
...
An existing webhook's signing secret cannot be read back by the API. Copy it from Stripe's endpoint details. For localhost, use Stripe CLI forwarding to `/api/billing/webhook` and use the CLI's displayed signing secret locally. Never mix live prices/customers with a test key.

Manual Stripe Dashboard setup is also supported: create the prices in the table below, copy their `price_…` IDs to the matching variables, and configure a customer portal with invoice history, payment method updates and cancellation **at period end**. Disable subscription updates, prorations and immediate plan switching. The backend intentionally accepts one complete recurring plan line per invoice; promotions, trials, coupons, metered prices, custom quantities and added invoice items are not supported in this release.
...
5. Run the setup script with live credentials only when ready to accept real subscriptions. Put the matching live IDs/secrets in Vercel Production environment variables. Preserve existing production Supabase/fal keys. Apply the production migration, set `CREDITS_ENABLED=true`, and redeploy.
